...
const parityMap = JSON.parse(await readFile(parityMapPath, "utf8"));
...
async function countMappedReferenceTests() {
  let count = 0;
...
function validateParityMap(actualTestCount) {
...
await runStep("Screen manifest validation", process.execPath, ["scripts/validate-manifests.mjs"]);
