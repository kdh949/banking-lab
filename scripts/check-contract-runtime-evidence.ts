...
for (const requiredDocMarker of [
  "extend springdoc/Jackson DTO schema generation beyond the core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service",
  "Kotlin controller source-to-OpenAPI path/method diffing is wired through `contracts:diff-openapi`",
  "core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes",
...
const evidence: ContractRuntimeEvidence = {
  ...
  status: "partial",
  statusReason:
    "Current gates prove checked-in OpenAPI/AsyncAPI structure, API-client operationId parity, Kotlin controller source-to-OpenAPI path/method parity, core-banking ledger command, inquiry, and case workflow subset DTO schema parity, payment-service, notification-service, and reporting-service DTO schema parity, event schema references, runtime envelope fixture/schema validation, and selected source envelope markers. Full core-banking springdoc/Jackson DTO schema generation remains a PLAN-required gap.",
  openApi: {
    ...
    generatedDtoDiffGate: {
      ...
      note:
        "Kotlin controller source-to-OpenAPI path/method diffing is wired, core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes, and core-banking still needs full springdoc/Jackson DTO schema parity."
    }
  },
  asyncApi: {
  ...
  overclaimGuards: [
    {
      ...
      status: "pass",
      details: "README and contract evidence distinguish core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service DTO schema parity from remaining full core-banking springdoc/Jackson DTO schema generation."
    },
    {
