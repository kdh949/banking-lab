...
test("contract runtime evidence boundary records structural passes and missing runtime gates", async () => {
  ...
  assert.match(evidence.openApi.generatedDtoDiffGate.note, /core-banking ledger command subset plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes/);
...
test("contract runtime evidence docs avoid DTO and runtime event overclaims", async () => {
  ...
  assert.match(doc, /Do not claim core-banking ledger command subset DTO schema parity is equivalent to full core-banking Spring\/Jackson\/springdoc DTO-level OpenAPI schema parity/);
