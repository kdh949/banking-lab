...
test("H8 generated governance artifacts are synthetic and complete", async () => {
  ...
  assert.equal(access.summary.principalCount, 15);
  assert.equal(access.summary.roleAssignmentCount, 19);
  assert.ok(access.summary.overPrivilegeFlagCount > 0);
  assert.ok(access.principals.some((principal) => principal.principalId === "service-account-notification-service-api" && principal.roles.includes("NOTIFICATION_SERVICE")));
  assert.ok(access.principals.some((principal) => principal.principalId === "service-account-reporting-service-api" && principal.roles.includes("REPORTING_ANALYST")));
