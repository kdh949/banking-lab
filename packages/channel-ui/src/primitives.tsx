...
export function ChannelShell({ appId, eyebrow, title, status, navigation = ["Overview", "Work queue", "Controls", "Help"], children }: ChannelShellProps) {
  return (
        ...
        <aside className="channel-sidebar" aria-label={`${eyebrow} navigation`}>
          ...
          <nav className="channel-nav" aria-label="Channel sections">
            {navigation.map((item, index) => (
              <span className={index === 0 ? "is-active" : undefined} key={item}>{item}</span>
            ))}
          </nav>
        </aside>
