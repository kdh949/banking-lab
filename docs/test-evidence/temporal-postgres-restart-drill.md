...
This closes live Compose PostgreSQL process restart coverage for all current synthetic Temporal workflow contracts. Node retirement remains blocked until host crash shapes, API/outbox deployed process-failure variants, non-synthetic passkey operations, evidence-refresh completion, and final retirement review are complete.
