...
- `admin-console` renders an API-backed reporting panel that calls
  ...
  unmasked payloads.
  The ADM-701 manifest also declares the reason-required
  `POST /api/reports/retention/sweeps` retention lifecycle command.
...
- `fds-aml-console` now has conditional `FDS-301` browser wiring for FDS parameter reads and future-effective `highAmountMinor` change-request smoke through the Spring parameter API client. In this local run the new API-backed parameter command test was skipped because `BANKING_LAB_E2E_API_BASE_URL` was not set.
- `ops-console` now has conditional `OPS-301` browser wiring for reconciliation parameter reads and future-effective `autoMatchToleranceMinor` change-request smoke through the Spring parameter API client. In this local run the new API-backed parameter command test was skipped because `BANKING_LAB_E2E_API_BASE_URL` was not set.
- `audit-console` now has conditional `AUD-201` browser wiring for audit parameter reads and future-effective `retentionYears` change-request smoke through the Spring parameter API client. In this local run the new API-backed parameter command test was skipped because `BANKING_LAB_E2E_API_BASE_URL` was not set.
- `admin-console` now has conditional `ADM-201` browser wiring for security parameter reads and future-effective `staffSessionTtlSeconds` change-request smoke through the Spring parameter API client. In this local run the new API-backed parameter command test was skipped because `BANKING_LAB_E2E_API_BASE_URL` was not set.
...
- The synthetic seed now includes dedicated transfer-limit, KYC, fee waiver, and transaction-correction command rows so the `LIM102`, `KYC101`, `FEE102`, and `LED103` browser smokes do not mutate existing account/customer read-model assertions.
...
- `npm run test:reporting-service:keycloak-service-token` passed after the retention update; the live synthetic Compose smoke proved `reporting-service-api` client-credentials tokens can catalog, generate, list, export rendered artifacts, and run a no-op retention sweep with simulator tokens disabled.
...
## 2026-06-05 WRK002 Operational Retry Queue Update

This update adds a Back Office staff-workstation read slice for operator exception visibility without mutating retry state.
...
- API-backed WRK002 browser retrieval against a live Spring API was not run because `BANKING_LAB_E2E_API_BASE_URL` was not configured for the local Playwright run.

## 2026-06-05 WRK003 Workflow Timeline Update

This update adds a Back Office staff-workstation read slice for workflow state visibility across workflow, approval, and audit records.

Changes:

- `GET /api/staff/workflows/{businessReferenceId}/timeline` returns a reason-required timeline for a business reference.
- The read path combines `workflow_events`, `operator_approvals`, and `audit_events` while excluding audit payload JSON and snapshot bodies from the response.
...
- `npm run packages:typecheck` passed.
- `npm run next:staff-terminal:typecheck` passed after replacing the unsupported local icon name.
...
- API-backed WRK003 browser retrieval against a live Spring API was not run because `BANKING_LAB_E2E_API_BASE_URL` was not configured for the local Playwright run.
...
## 2026-06-04 FEE102 Fee Waiver Update
...
- `npm run test:core-banking:integration -- --tests lab.banking.core.staff.StaffAccessApiParityIntegrationTest` compiled the integration sources, then failed at Testcontainers initialization because Docker provider discovery failed locally. The report showed `/var/run/docker.sock` was missing.
- API-backed FEE102 browser execution against a live Spring API was not run locally because `BANKING_LAB_E2E_API_BASE_URL` was not configured.

## 2026-06-04 LED103 Transaction Correction Update

This update completes the remaining Phase B staff-command vertical slice from `docs/codex/implementation_missing_features_goals.md`.

Changes:

- `db/migrations/V017__transaction_correction_requests.sql` adds durable transaction correction request state with idempotency keys and a ledger transaction reference.
- `POST /api/staff/transactions/{transactionId}/correction-requests` creates `TRANSACTION_CORRECTION` maker-checker approvals.
...
- `npm run validate:manifests` passed with 68 manifests.
- `npm run packages:typecheck` passed.
- `npm run next:staff-terminal:typecheck` passed.
- `scripts/run-core-banking-tests.sh :services:core-banking:compileKotlin :services:core-banking:compileIntegrationTestKotlin` passed after sandbox escalation.
- `npm test` passed with 131 Node reference/oracle tests.
...
- `npm run test:core-banking:integration -- --tests lab.banking.core.security.SecurityAuthorizationIntegrationTest` passed after sandbox escalation.
- `docker compose config` passed.
- `docker compose --profile platform config` passed.
- `npm run evidence:refresh-check` passed.
...
- API-backed LED103 browser execution against a live Spring API was not run locally because `BANKING_LAB_E2E_API_BASE_URL` was not configured.

## 2026-06-04 PRD102 Deposit Product Interest Update

This update adds the first Phase C product-ledger vertical slice from `docs/codex/implementation_missing_features_goals.md`.

Changes:

- `db/migrations/V018__deposit_products_interest.sql` adds deposit products, interest rate versions, account product enrollments, interest accruals, interest posting batches, deposit rate change requests, and the `INTEREST` posting type.
- `GET /api/products/deposits` and `GET /api/products/deposits/{productId}` expose the synthetic deposit product catalog and active rate version.
- `POST /api/staff/products/deposits/{productId}/rate-change-requests` creates `PRODUCT_PARAMETER_CHANGE` maker-checker approvals.
- `POST /api/staff/approvals/{approvalId}/approve` applies approved rate changes as new effective-date rate versions without mutating historical ledger rows.
- `POST /api/ops/interest-accruals/run` creates deterministic daily interest accrual rows for enrolled active accounts.
- `POST /api/ops/interest-posting-batches` posts calculated interest through a balanced `INTEREST_POSTING` ledger transaction and stores idempotent batch evidence.
- The shared api-client, `PRD-101`, `PRD-102`, `OPS-401`, `OPS-402` manifests, synthetic seed, and staff-terminal renderer expose the product/interest slice. The `PRD102` browser smoke remains conditional on a configured Spring API.

Commands run:

- `npm run validate:manifests` passed with 72 manifests.
- `npm run packages:typecheck` passed.
- `npm run next:staff-terminal:typecheck` passed.
...
- Customer transfer retry/failure, customer transfer history/held-status, customer held/failed status parity, customer complaint entry, customer complaint confirmation, staff privileged unmask, staff customer change approval, complaint answer approval, FDS release approval, FDS block approval, AML closure approval, reconciliation adjustment approval, and complaint/FDS/AML/reconciliation workflow failure-states have browser evidence; FDS-301, OPS-301, AUD-201, and ADM-201 parameter command wiring has local shell evidence and remains API-run conditional; complaint answer/failure, ops reconciliation adjustment/failure, audit hash-chain read-model, and FDS/AML risk command/failure paths also have live Keycloak evidence.
