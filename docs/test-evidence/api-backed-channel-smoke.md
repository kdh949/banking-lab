...
- Customer transfer retry/failure, customer transfer history/held-status, customer held/failed status parity, customer complaint entry, customer complaint confirmation, staff privileged unmask, staff customer change approval, complaint answer approval, FDS release approval, FDS block approval, AML closure approval, reconciliation adjustment approval, and complaint/FDS/AML/reconciliation workflow failure-states have browser evidence; FDS-301 parameter command wiring has local shell evidence and remains API-run conditional; complaint answer/failure, ops reconciliation adjustment/failure, audit hash-chain read-model, and FDS/AML risk command/failure paths also have live Keycloak evidence.
