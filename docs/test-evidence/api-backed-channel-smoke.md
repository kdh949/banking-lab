...
- `admin-console` now has conditional `ADM-301` browser wiring for authorization parameter reads and future-effective `reasonRequiredScreens` change-request smoke through the Spring parameter API client. In this local run the new API-backed parameter command test was skipped because `BANKING_LAB_E2E_API_BASE_URL` was not set.
