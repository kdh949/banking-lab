...
- `fds-aml-console` executes a live Spring API-backed `FDS-301` browser parameter command smoke for a future-effective `highAmountMinor` change request through `npm run test:parameter-command-e2e-compose`.
- `ops-console` executes a live Spring API-backed `OPS-301` browser parameter command smoke for a future-effective `autoMatchToleranceMinor` change request through `npm run test:parameter-command-e2e-compose`.
- `audit-console` executes a live Spring API-backed `AUD-201` browser parameter command smoke for a future-effective `retentionYears` change request through `npm run test:parameter-command-e2e-compose`.
- `admin-console` executes a live Spring API-backed `ADM-201` browser parameter command smoke for a future-effective `staffSessionTtlSeconds` change request through `npm run test:parameter-command-e2e-compose`.
- `admin-console` executes a live Spring API-backed `ADM-301` browser parameter command smoke for a future-effective `reasonRequiredScreens` change request through `npm run test:parameter-command-e2e-compose`.
