...
- `NEXT_PUBLIC_BANKING_API_BASE_URL=http://127.0.0.1:18126 NEXT_PUBLIC_BANKING_KEYCLOAK_BASE_URL=http://localhost:18127 NEXT_PUBLIC_BANKING_SIMULATOR_TOKENS_ENABLED=false BANKING_LAB_KEYCLOAK_BASE_URL=http://localhost:18127 npm run next:staff-terminal` initially failed in the sandbox because port `3002` could not be bound; the same command passed under the approved execution path.
- Browser inspection of `http://localhost:3002` showed `simulator token smoke disabled` for simulator-token panels and one enabled `Sign in WebAuthn manager with Keycloak` button.
...
- The staff-terminal manual passkey mode disables simulator-token smoke calls with `NEXT_PUBLIC_BANKING_SIMULATOR_TOKENS_ENABLED=false`, preventing expected 401 noise while preserving the Keycloak WebAuthn entry point.
- The passkey preflight now locks the staff-terminal manual mode and WebAuthn spec as explicit passkey/evidence-refresh evidence.
