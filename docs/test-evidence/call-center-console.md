...
- `ApiBackedCallCenterPanel` can run a signed agent/manager Keycloak workflow smoke when both `NEXT_PUBLIC_BANKING_API_BASE_URL` and `NEXT_PUBLIC_BANKING_KEYCLOAK_BASE_URL` are configured.
...
- The call-center Keycloak client, users, token exchange route, and env-gated Playwright smoke are implemented, but no live full-stack Keycloak/JWKS browser execution has been recorded yet.
