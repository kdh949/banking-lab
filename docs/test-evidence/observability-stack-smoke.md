...
This closes the local observability readiness, Prometheus scrape, Loki ingestion, Loki ruler alert, and Grafana dashboard validation blockers for the target stack. Node retirement remains blocked until host crash shapes, API/outbox deployed process-failure variants, non-synthetic passkey operations, evidence-refresh completion, and final retirement review are complete.
