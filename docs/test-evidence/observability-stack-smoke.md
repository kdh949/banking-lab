...
This closes the local observability readiness, Prometheus scrape, Loki ingestion, Loki ruler alert, and Grafana dashboard validation blockers for the target stack. Node retirement remains blocked until host crash shapes, API process crash after durable ledger/outbox commit, outbox tracing, non-synthetic passkey operations, evidence-refresh completion, and final retirement review are complete.
