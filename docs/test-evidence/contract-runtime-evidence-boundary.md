...
- `npm run contracts:diff-openapi` validates Kotlin `@RestController` `/api/**` and checked-in `/health` paths/methods against checked-in OpenAPI contracts, generates core-banking ledger command subset plus payment-service, notification-service, and reporting-service DTO schemas from Kotlin data classes, and writes generated source snapshots under `docs/test-evidence/generated/openapi/`.
...
- Kotlin controller source-to-OpenAPI path/method diffing is wired through `contracts:diff-openapi`, and core-banking ledger command subset plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes. Do not claim core-banking ledger command subset DTO schema parity is equivalent to full core-banking Spring/Jackson/springdoc DTO-level OpenAPI schema parity.
