...
Status: partial. The repository has structural OpenAPI, API-client, Kotlin controller source-to-OpenAPI path/method diffing, AsyncAPI, event-schema gates, and runtime envelope fixture/schema/source-marker validation. It does not yet have PLAN-required springdoc/Jackson DTO schema generation.
