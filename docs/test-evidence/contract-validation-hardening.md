...
- Core-banking OpenAPI still uses generic response schemas for broad operation coverage outside the ledger command, inquiry, and case workflow subsets; Kotlin controller source-to-OpenAPI path/method diffing is wired through `contracts:diff-openapi`, and core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes. Full core-banking springdoc/Jackson DTO schema generation remains a future improvement.
...
- The gate fails on runtime path/methods missing from OpenAPI, OpenAPI
  ...
  schema reference.
- For the core-banking ledger command, inquiry, and case workflow subsets plus payment-service, notification-service, and reporting-service, the gate also fails if a runtime request/response DTO
...
- This is still not a full core-banking springdoc/Jackson schema export.
  Existing broad core-banking `AnyJsonResponse` coverage remains a documented
  residual risk until DTO schema generation is extended there.

Current local validation:

| Command | Result | Notes |
| --- | --- | --- |
| `npm run contracts:diff-openapi` | pass | Matched 142 core-banking, 14 payment-service, 16 notification-service, and 6 reporting-service controller operations to checked-in OpenAPI, and generated 66 core-banking ledger command, inquiry, and case workflow subset, 24 payment-service, 17 notification-service, and 12 reporting-service DTO/enum schemas from Kotlin data classes. |
