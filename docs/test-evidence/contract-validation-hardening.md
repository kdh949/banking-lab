...
- Core-banking OpenAPI still uses generic response schemas for broad operation coverage outside the ledger command subset; Kotlin controller source-to-OpenAPI path/method diffing is wired through `contracts:diff-openapi`, and core-banking ledger command subset plus payment-service, notification-service, and reporting-service DTO schemas are generated from Kotlin data classes. Full core-banking springdoc/Jackson DTO schema generation remains a future improvement.
...
- The gate fails on runtime path/methods missing from OpenAPI, OpenAPI
  ...
  schema reference.
- For the core-banking ledger command subset plus payment-service, notification-service, and reporting-service, the gate also fails if a runtime request/response DTO
