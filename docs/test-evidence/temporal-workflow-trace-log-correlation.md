...
This evidence does not claim external alert routing, every workflow transition, host crash-shape coverage, broader database/process-failure variants, or propagation from a browser/API request into the Temporal workflow trace. Loki ingestion, local alert evaluation, and dashboard provisioning are covered separately in `docs/test-evidence/observability-stack-smoke.md`.
...
This closes workflow-specific trace/log correlation for Temporal signal and approval-completion events across the current banking case types, plus live rejection and self-approval failure transitions. Node retirement remains blocked until host crash shapes and broader database/process-failure variants, non-synthetic passkey operations, evidence-refresh completion, and final retirement review are complete.
