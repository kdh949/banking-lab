...
| Staff integrated terminal | 7/10 | partial | Current `staff-terminal` is the iWorks integrated shell with `/api/terminal-status` plus a bounded Spring API evidence panel for reason-required `staffCustomerDetail` and approval-inbox reads. Staff control commands remain Spring-backed and tested separately: `docs/implementation-coverage-matrix.md`, `docs/test-evidence/api-backed-channel-smoke.md`, `docs/test-evidence/live-route-api-execution.md` | The official staff frontend still does not restore the retired accounts/approvals/audit/customers/tx/workflows route set, and the new API panel is a scoped evidence surface rather than full operator workflow coverage. | Run `npm run test:staff-terminal:api-e2e-compose` before portfolio recording and add only targeted operator command paths that close demo/control gaps. |
...
| PLAN completion audit | 7/10 | partial | `npm run portfolio:completion-audit` now checks the PLAN final conditions separately from the older Node retirement `goal:completion-audit` gate and writes `docs/test-evidence/generated/portfolio-completion-audit.json`; #88 adds `docs/test-evidence/final-command-refresh-2026-06-11.md` for the local final command refresh. | The audit is intentionally `not-complete` while hosted CI is blocked and route-to-API evidence remains partial. | Use `npm run portfolio:completion-audit -- --require-complete` only as a final gate after #83 and the remaining live-route evidence row are closed. |
