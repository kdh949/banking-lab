...
| Staff integrated terminal | 7/10 | partial | Current `staff-terminal` is the iWorks integrated shell with `/api/terminal-status` plus a bounded Spring API evidence panel for reason-required `staffCustomerDetail` and approval-inbox reads. Staff control commands remain Spring-backed and tested separately: `docs/implementation-coverage-matrix.md`, `docs/test-evidence/api-backed-channel-smoke.md`, `docs/test-evidence/live-route-api-execution.md` | The official staff frontend still does not restore the retired accounts/approvals/audit/customers/tx/workflows route set, and the API panel is a scoped evidence surface rather than full operator workflow coverage. | Keep rerunning `npm run test:staff-terminal:api-e2e-compose` before release recording and add only targeted operator command paths that close demo/control gaps. |
| Customer channel | 8/10 | strong | Customer signup/login, accounts/detail/history, internal transfer, complaint entry, loan/card/payment/notification panels, and live synthetic Compose route/API evidence are documented in `docs/test-evidence/customer-onboarding-self-service.md`, `docs/test-evidence/api-backed-channel-smoke.md`, and `docs/test-evidence/live-route-api-execution.md` | Broader Keycloak browser smokes remain environment-gated; skipped Playwright tests are not pass evidence. | Rerun customer-web API/Keycloak smokes against a fresh synthetic stack before release evidence. |
...
| Hosted CI | 3/10 | blocked | `.github/workflows/ci.yml` has the required jobs and `npm run ci:check-workflow` verifies the structure; PR #82 and the post-merge main push are recorded in `docs/test-evidence/ci-hosted-run-status.md` with `runner_id: 0` and `steps: 0`, and the infrastructure blocker is tracked in #83. | GitHub Actions jobs did not start because of runner allocation/billing-spending-limit failure, so hosted CI is not green. | Resolve #83, rerun CI, and record run URLs without treating local fallback as hosted success. |
...
| PLAN completion audit | 8/10 | blocked | `npm run portfolio:completion-audit` now checks the PLAN final conditions separately from the older Node retirement `goal:completion-audit` gate and writes `docs/test-evidence/generated/portfolio-completion-audit.json`; #88 adds `docs/test-evidence/final-command-refresh-2026-06-11.md` and #90 adds live customer/staff route-to-API Compose pass evidence. | The audit is intentionally `not-complete` while hosted CI is blocked before runner startup. | Use `npm run portfolio:completion-audit -- --require-complete` only as a final gate after #83 is resolved and hosted jobs execute. |
