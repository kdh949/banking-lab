...
| Call-center 상담 전산 | 2/10 | missing | Adjacent staff access, masking, reason-required audit, complaint, workflow timeline, and approval APIs exist, but no dedicated call-center console or complete staff-terminal call-center workflow is implemented. Evidence: `docs/test-evidence/live-route-api-execution.md` and `docs/test-evidence/evidence-gap-report.md` | PLAN-required 상담 이력, 상담 메모, 후처리, escalation, authorization, and audit workflow is still missing as a coherent API-backed operator journey. | Implement a synthetic-only call-center workflow with masked notes, reason-required access, after-call tasks, escalation, role policy, audit, and browser/API evidence. |
...
Use `docs/demo-scenarios/demo-video-script.md` as the portfolio walkthrough. The script intentionally separates live/API-backed paths from blocked or missing paths. The call-center scene is a gap disclosure until a target-stack call-center workflow is implemented.
