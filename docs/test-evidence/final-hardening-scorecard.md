...
| PLAN completion audit | 6/10 | partial | `npm run portfolio:completion-audit` now checks the PLAN final conditions separately from the older Node retirement `goal:completion-audit` gate and writes `docs/test-evidence/generated/portfolio-completion-audit.json`. | The audit is intentionally `not-complete` while hosted CI is blocked, route-to-API evidence remains partial, and the final command refresh has not been rerun in one release pass. | Use `npm run portfolio:completion-audit -- --require-complete` only as a final gate after #83 and the remaining partial evidence rows are closed. |
