# Customer Self-Service 360 Hardening Evidence
...
- `missing session` `/api/customer/me` and `/api/customer/360` requests fail with structured `AUTHORIZATION_POLICY_VIOLATION` responses before profile or 360 view audit rows are created.
...
- JDK 21 validation was executed with Homebrew OpenJDK 21 because the default macOS Java home resolves to unsupported JDK 26.0.1.
- `JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home ./gradlew :services:core-banking:compileKotlin`: passed.
- `JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home npm run test:core-banking:integration -- --tests '*Customer*' --tests '*AccountOpening*' --tests '*Statement*'`: passed.
...
- `npm run test:e2e -- apps/customer-web/e2e/customer-self-service-360.spec.ts --project=chromium`: skipped 1 live API smoke because live environment variables were not configured.
...
## Known Skips And Environment Notes

- Default macOS Java discovery still resolves Temurin JDK 26.0.1; Gradle/Kotlin must be run with `JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home` until the default Java home is changed.
