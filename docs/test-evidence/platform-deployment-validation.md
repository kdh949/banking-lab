...
- `npm run k8s:validate` checked 20 required `infra/k8s` files and 24 Kubernetes resources. The current rerun had no reachable local cluster, so `docs/test-evidence/generated/k8s-validation.json` records `kubectlClientDryRun.status = "skipped_no_cluster"` while structural validation passed.
- `npm run helm:template` used the local Helm CLI and validated 18 rendered resources.
