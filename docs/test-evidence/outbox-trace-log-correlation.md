...
This closes outbox worker trace/log correlation evidence for the current synthetic target-stack outbox publish path. Node retirement remains blocked until host crash shapes, non-synthetic passkey operations, evidence-refresh completion, and final retirement review are complete.
