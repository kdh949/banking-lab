...
- Admin-console has now been added as the seventh target Next surface with manifest-rendered platform-control and privileged security-parameter screens, a Spring `/api/admin/platform/summary` route, and `security-admin01` Keycloak propagation hook. Keep evidence-refresh open until its live browser API/Keycloak smoke is rerun and recorded with the rest of the channel evidence.
