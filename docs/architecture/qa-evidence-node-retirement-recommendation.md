...
| `non-synthetic-passkey-operations` | Pending | Keep pending; `docs/test-evidence/passkey-non-synthetic-operations.md` now records the boundary and `npm run node:retirement-gate` rejects any pass claim that lacks a real platform or hardware security-key evidence artifact. Chromium CDP virtual-authenticator smoke remains useful WebAuthn evidence but is not sufficient for this gate. |
