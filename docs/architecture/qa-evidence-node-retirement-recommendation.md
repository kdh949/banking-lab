...
The correct current status is blocked even though all 42 mapped parity scenarios now pass in target-backed evidence and the API-backed channel parity gate is now closed. The remaining blockers are host crash shapes, API/outbox deployed process-failure variants, non-synthetic passkey operations, evidence-refresh completion, and final retirement review required by `PLAN.md` and `AGENTS.md`.
...
| `evidence-refresh` | In progress | Keep in progress until full target command outputs, host crash shapes, and API/outbox deployed process-failure variants are captured. |
