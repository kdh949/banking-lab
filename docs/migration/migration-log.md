...
## 2026-06-03: Passkey Manual Readiness Guard

Changes completed:

- Added `scripts/check-passkey-manual-readiness.ts` and `npm run passkey:evidence:readiness`.
- The readiness checker validates the prepared local passkey templates under `tmp/passkey-evidence-manual/` against the documented manual-run boundary before the real browser ceremony.
- The checker confirms the command template keeps TODO status/exit-code placeholders, the manual Compose command disables simulator tokens, the local Keycloak issuer is `http://localhost:18127/realms/banking-lab`, Spring health uses `http://127.0.0.1:18126/health`, the recorder command points at the prepared redacted inputs, and the staff-panel template includes masked PII plus audit markers.
- Added `tests/passkeyManualReadiness.test.mjs` to prove freshly prepared platform and hardware-key templates pass readiness, missing templates fail, simulator-token drift fails, and virtual-authenticator command drift fails.
- Updated the passkey preflight, evidence-refresh checker, retirement gate, passkey evidence boundary, and evidence-refresh review to track the readiness command and test.
- Adjusted the passkey prepare script so manual-authenticator summaries remain replacement instructions until the real ceremony is complete.

Verification:

- `npm run scripts:typecheck` passed.
- `npm run passkey:evidence:prepare` passed and generated ignored TODO templates under `tmp/passkey-evidence-manual/`.
- `node --test tests/passkeyManualReadiness.test.mjs tests/passkeyEvidencePreflight.test.mjs tests/passkeyEvidencePrepare.test.mjs` initially failed because the prepared manual-authenticator summary read like completed evidence; after changing the summary to a replacement instruction it passed 11 tests.
...
- The future manual passkey run now has a machine-checked readiness step between template preparation and evidence recording.
- The readiness step catches local command/port/issuer/simulator-token drift before the real platform-authenticator or hardware-security-key ceremony.
...
## 2026-06-03: Final Review Passkey Artifact Verification Guard

Changes completed:

- Strengthened `scripts/record-final-retirement-review.ts` so it runs `scripts/verify-passkey-non-synthetic-evidence.ts` before writing any final review artifact.
- Strengthened `scripts/verify-final-retirement-review.ts` so final review artifacts only pass when the referenced passkey evidence artifact also passes strict verification.
- Updated `scripts/check-node-retirement-ready-simulation.ts` so its final-review fixture references the same temporary passkey artifact that the retirement gate verifies.
- Updated final-review recorder and verifier tests to include verified passkey evidence fixtures and reject missing passkey artifacts.
- Updated `docs/test-evidence/final-retirement-review-verifier.md` and `docs/test-evidence/evidence-refresh-review.md` to document the direct passkey artifact verifier dependency.
...
- `npm run validate:manifests` validated 28 manifests.
- `npm run evidence:pack` passed and regenerated the evidence pack summary without tracked changes.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 111 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- Future final retirement review evidence cannot be recorded or verified unless the passkey artifact itself passes strict verification.
- The ready-state simulation proves the stricter final-review/passkey artifact coupling can still satisfy the future ready gate.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Final Review Command Evidence Object Guard

Changes completed:

- Strengthened `scripts/record-final-retirement-review.ts` so every item in the final-review command evidence JSON array must be an object.
- Strengthened `scripts/verify-final-retirement-review.ts` so final-review artifacts reject non-object command entries instead of silently filtering them out.
- Added recorder and verifier tests for hidden string command evidence entries.
- Updated `docs/test-evidence/final-retirement-review-verifier.md` and `docs/test-evidence/evidence-refresh-review.md` to document the non-object command evidence rejection.
...
- Future final retirement review pass artifacts can no longer hide unstructured command evidence entries in the commands array.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Structured Passkey Command Evidence Guard

Changes completed:

- Strengthened `scripts/record-passkey-non-synthetic-evidence.ts` so future passkey command evidence must be a JSON array of objects with `command`, `status: "pass"`, `exitCode: 0`, and a non-empty `summary`.
- Strengthened `scripts/verify-passkey-non-synthetic-evidence.ts` and `scripts/check-node-retirement-gate.ts` so passkey artifacts reject string-only command lists, failed command entries, duplicate commands, and empty summaries.
- Updated `scripts/check-node-retirement-ready-simulation.ts` so the future ready-state fixture uses the structured passkey command evidence schema.
- Updated `scripts/check-passkey-non-synthetic-preflight.ts`, `tests/passkeyEvidenceRecorder.test.mjs`, `tests/passkeyEvidenceVerifier.test.mjs`, and passkey evidence docs to lock the new schema and failure cases.
...
- `npm run validate:manifests` validated 28 manifests.
- `npm run evidence:pack` passed and regenerated the evidence pack summary without tracked changes.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 109 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- Future non-synthetic passkey pass artifacts can no longer hide failed or duplicate command evidence behind a redacted command list.
- The ready-state simulation proves the stricter passkey artifact schema can still satisfy the future ready gate.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Retirement Review Preflight Final Verifier Guard

Changes completed:

- Strengthened `scripts/check-retirement-review-preflight.ts` so it runs the strict final retirement review verifier while the review gate is pending and requires the verifier to fail against the missing default artifact path.
- Updated `tests/retirementReviewPreflight.test.mjs` to prove the preflight output and wiring include the strict final review verifier guard.
- Updated `docs/test-evidence/node-retirement-review.md` and `docs/test-evidence/final-retirement-review-verifier.md` to document the pending-state fail-closed verifier behavior.

Verification:

- `npm run scripts:typecheck` passed.
- `node --test tests/retirementReviewPreflight.test.mjs tests/finalRetirementReviewVerifier.test.mjs tests/goalCompletionAudit.test.mjs tests/evidenceRefresh.test.mjs` passed 12 tests.
- `npm run retirement:review-preflight` passed and reported the strict final review verifier as part of the consistent checks.
- `npm run goal:completion-audit` passed with `Goal completion audit: not complete`.
- `npm run evidence:refresh-check` passed.
- `npm run node:retirement-gate` passed with blocked status and listed only `non-synthetic-passkey-operations` plus `retirement-review` as incomplete.
- `npm run retirement:audit` passed with blocked status, 60 approved-reference `.mjs` files, 37 target anchors, 316 evidence paths, and 42/42 mapped scenarios.
...
- `npm run validate:manifests` validated 28 manifests.
- `npm run evidence:pack` passed and regenerated the evidence pack summary without tracked changes.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 107 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- The final review preflight now proves the final review verifier is executable and fail-closed before any future ready claim.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Goal Completion Final Review Artifact Guard

Changes completed:

- Strengthened `scripts/check-goal-completion-audit.ts` so `retirement-review: pass` is accepted only when `docs/test-evidence/generated/final-node-retirement-review.json` exists and `scripts/verify-final-retirement-review.ts` reports pass.
- Updated `tests/goalCompletionAudit.test.mjs` to prove the completion audit is wired to the strict final retirement review verifier.
- Updated `docs/test-evidence/goal-completion-audit.md` to document the final review artifact guard and the missing generated review artifact blocker.

Verification:

- `npm run scripts:typecheck` passed.
- `node --test tests/goalCompletionAudit.test.mjs tests/finalRetirementReviewVerifier.test.mjs tests/nodeRetirementReadySimulation.test.mjs tests/retirementReviewPreflight.test.mjs tests/evidenceRefresh.test.mjs` passed 14 tests.
- `npm run goal:completion-audit` passed with `Goal completion audit: not complete`; it now reports `gate:retirement-review` blocked because the final review artifact is missing or the gate is not pass.
- `npm run evidence:refresh-check` passed.
- `npm run retirement:review-preflight` passed.
- `npm run node:retirement-gate` passed with blocked status and listed only `non-synthetic-passkey-operations` plus `retirement-review` as incomplete.
- `npm run retirement:audit` passed with blocked status, 60 approved-reference `.mjs` files, 37 target anchors, 316 evidence paths, and 42/42 mapped scenarios.
...
- The active-goal completion audit now independently verifies final retirement review evidence instead of relying only on the retirement gate status.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Final Retirement Review Verifier

Changes completed:

- Added `scripts/verify-final-retirement-review.ts` and `npm run retirement:final-review:verify`.
- Added `tests/finalRetirementReviewVerifier.test.mjs` to validate complete final-review artifacts and reject missing commands, failed controls, remaining blockers, unredacted secrets, and a missing default artifact.
- Added `docs/test-evidence/final-retirement-review-verifier.md` with the final-review artifact boundary.
- Wired the verifier document, script, and test into `evidence-refresh` and `retirement-review` gate evidence.
- Extended `npm run node:retirement-gate` so a future `retirement-review: pass` state must also pass strict final-review artifact verification before the gate can be ready.
- Updated the final retirement review runbook to require `docs/test-evidence/generated/final-node-retirement-review.json`.

Verification:

- `npm run scripts:typecheck` passed.
- `node --test tests/finalRetirementReviewVerifier.test.mjs tests/evidenceRefresh.test.mjs tests/retirementReviewPreflight.test.mjs tests/migrationFoundation.test.mjs tests/goalCompletionAudit.test.mjs` passed 18 tests.
- `npm run retirement:review-preflight` passed.
- `npm run evidence:refresh-check` passed.
- `npm run node:retirement-gate` passed with blocked status and listed only `non-synthetic-passkey-operations` plus `retirement-review` as incomplete.
...
- `npm run goal:completion-audit` passed with `Goal completion audit: not complete`.
- `git diff --check` passed.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 97 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- The final retirement review now has a strict generated-artifact verifier for the future pass state.
- The Node retirement gate stays fail-closed if final review is marked pass without a concrete post-passkey command/control artifact.
- This does not mark Node retirement ready.
...
- Added `scripts/check-goal-completion-audit.ts` and `npm run goal:completion-audit`.
- Added `tests/goalCompletionAudit.test.mjs` to verify that the audit reports the current objective as not complete and fails closed with `--require-complete`.
- Added `docs/test-evidence/goal-completion-audit.md` as the current completion-blocked evidence artifact.
- Wired the completion audit document, script, and test into `evidence-refresh` and `retirement-review` evidence.
- Extended `npm run retirement:review-preflight` so final review preflight delegates to the completion audit.
- The audit checks the area stack audit, mapped parity, required pass gates, strict passkey verifier when applicable, and `npm run node:retirement-gate` readiness output before accepting completion.

Verification:

- `npm run goal:completion-audit` passed with `Goal completion audit: not complete`.
- `node --test tests/goalCompletionAudit.test.mjs tests/evidenceRefresh.test.mjs tests/retirementReviewPreflight.test.mjs` passed 7 tests.
- `npm run scripts:typecheck` passed.
- `npm run evidence:refresh-check` passed.
- `npm run retirement:review-preflight` passed.
- `npm run retirement:stack-audit` passed.
...
- `npm run evidence:pack` passed and regenerated the evidence pack summary.
- `git diff --check` passed.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 91 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- The active goal now has a machine-checkable completion audit that refuses to treat the objective as complete while passkey evidence, final retirement review, or ready retirement gate evidence is missing.
- `npm run goal:completion-audit -- --require-complete` remains expected to fail until the remaining blockers are genuinely resolved.
...
## 2026-06-03: Goal Completion Ready-State Simulation Coupling

Changes completed:

- Extended `scripts/check-goal-completion-audit.ts` so the active-goal completion audit runs the Node retirement ready-state simulation before accepting the future completion path.
- Updated `tests/goalCompletionAudit.test.mjs` to assert that completion output includes `retirement-ready-state-simulation: pass` while the current result remains `Goal completion audit: not complete`.
- Updated `docs/test-evidence/goal-completion-audit.md` to document the ready-state simulation as a required completion precondition that does not alter the real blocked gate.

Verification:

- `npm run scripts:typecheck` passed.
- `node --test tests/goalCompletionAudit.test.mjs tests/nodeRetirementReadySimulation.test.mjs tests/evidenceRefresh.test.mjs tests/retirementReviewPreflight.test.mjs` passed 9 tests.
- `npm run goal:completion-audit` passed with `Goal completion audit: not complete` and `retirement-ready-state-simulation: pass`.
- `npm run evidence:refresh-check` passed.
- `npm run retirement:review-preflight` passed.
- `npm run node:retirement-gate` passed with blocked status and listed only `non-synthetic-passkey-operations` plus `retirement-review` as incomplete.
- `npm run retirement:audit` passed with blocked status, 60 approved-reference `.mjs` files, 37 target anchors, 316 evidence paths, and 42/42 mapped scenarios.
...
- The goal completion audit now fails if the fixture-proven future ready path regresses.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Passkey Live Command Evidence Guard

Changes completed:

- Strengthened `scripts/record-passkey-non-synthetic-evidence.ts` so future passkey evidence commands must include live Compose platform startup, `BANKING_LAB_SECURITY_SIMULATOR_TOKENS_ENABLED=false`, Keycloak discovery readiness, Spring `/health` readiness, a real platform/hardware authenticator attestation, and `npm run passkey:evidence:record`.
- Strengthened `scripts/verify-passkey-non-synthetic-evidence.ts` with the same command-evidence requirements and an explicit rejection for CDP/virtual-authenticator command markers.
- Updated `scripts/check-node-retirement-ready-simulation.ts` so its fixture passkey artifact satisfies the same stricter future-ready path.
- Added recorder/verifier tests for incomplete live command evidence.
- Updated `scripts/check-passkey-non-synthetic-preflight.ts`, `docs/test-evidence/passkey-non-synthetic-operations.md`, and `docs/test-evidence/evidence-refresh-review.md` to lock the new command evidence boundary.
...
- `npm run validate:manifests` validated 28 manifests.
- `npm run evidence:pack` passed and regenerated the evidence pack summary without tracked changes.
- Initial sandboxed `npm run parity` failed because Node reference tests could not bind `127.0.0.1` (`listen EPERM`). The same command passed under the approved execution path with 105 Node reference/structural tests, 28 manifest validations, 6 screen-engine tests, and evidence pack generation.

Result:

- Future non-synthetic passkey evidence can no longer pass with only a generic manual assertion and recorder command.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Final Review Preflight Command Evidence Guard

Changes completed:

- Extended `scripts/record-final-retirement-review.ts` and `scripts/verify-final-retirement-review.ts` so final retirement review pass artifacts must include post-passkey command evidence for `npm run retirement:ready-simulate`, `npm run passkey:evidence:preflight`, and `npm run retirement:review-preflight`.
- Updated `scripts/check-node-retirement-ready-simulation.ts` so its final-review fixture satisfies the same stronger required-command list.
- Updated final-review recorder/verifier tests and `docs/test-evidence/final-retirement-review-verifier.md`.
- Updated `docs/test-evidence/node-retirement-review.md` and `docs/test-evidence/evidence-refresh-review.md` so the runbook and evidence-refresh summary match the stricter verifier contract.

Verification:

- `npm run scripts:typecheck` passed.
- `node --test tests/finalRetirementReviewRecorder.test.mjs tests/finalRetirementReviewVerifier.test.mjs tests/nodeRetirementReadySimulation.test.mjs tests/retirementReviewPreflight.test.mjs tests/evidenceRefresh.test.mjs` passed 14 tests.
- `npm run retirement:ready-simulate` passed.
- `npm run retirement:review-preflight` passed.
- `npm run goal:completion-audit` passed with `Goal completion audit: not complete`.
- `npm run evidence:refresh-check` passed.
...
- Future final retirement review evidence now proves the passkey, ready-state, and review preflight checks were rerun after passkey evidence, not just the broad parity/boundary commands.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
...
## 2026-06-03: Final Review Command Evidence Consistency Guard

Changes completed:

- Strengthened `scripts/record-final-retirement-review.ts` so every command evidence item in a final-review pass artifact must have a non-empty command, `status: "pass"`, `exitCode: 0`, `runAfterPasskeyEvidence: true`, and a non-empty summary.
- Strengthened `scripts/verify-final-retirement-review.ts` with the same all-command validation.
- Added duplicate-command rejection so a pass artifact cannot carry multiple evidence entries for the same command.
- Added recorder and verifier tests that reject duplicate `npm run parity` evidence and extra failed command evidence.
- Updated `docs/test-evidence/final-retirement-review-verifier.md` and `docs/test-evidence/evidence-refresh-review.md` to document the consistency guard.
...
- Future final retirement review pass evidence can no longer hide failed or duplicate command evidence alongside the required passing commands.
- The real Node retirement gate remains blocked until non-synthetic passkey evidence and final retirement review artifacts exist and pass verification.
- This does not mark Node retirement ready.
