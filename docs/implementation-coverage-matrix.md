...
| staff-terminal | Fee waiver request/approval/rejection: `FEE-102` | yes | yes | yes | yes | `fee_waiver_requests` | branch/manager/call-center request roles; manager/compliance checker roles | request/reject/approve/execute audit with `feePostingCreated=false` | yes | Spring integration test passed locally for StaffAccess suite; conditional Playwright smoke added | this matrix and `docs/test-evidence/api-backed-channel-smoke.md` | api-backed |
...
- `npm run validate:manifests`: pass, 68 manifests after Phase B LED-103 update.
...
- `npm run test:e2e -- apps/staff-terminal/e2e/staff-terminal-parity.spec.ts`: pass after Phase B LED-103 update, 5 passed and 12 skipped because API/Keycloak variables were not configured.
- `npm run test:e2e`: pass after Phase B LED-103 update, 17 passed and 39 skipped because API/Keycloak variables were not configured. A first parallel attempt in a previous PR failed with port 3001 already in use and is not counted as product failure.
...
2. Phase C should add product/fee/interest tables next, so fee posting/refund and fee waiver policy share one approval model.
