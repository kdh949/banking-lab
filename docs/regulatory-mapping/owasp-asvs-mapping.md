...
| V4 Access Control | role declarations in screen manifests, Spring route role gates, customer ownership check, manager-only approval policy, and high-risk command step-up/device/session policy | `screen-manifests`, manifest validation, `SecurityAuthorizationIntegrationTest`, `JwksAuthorizationIntegrationTest` |
