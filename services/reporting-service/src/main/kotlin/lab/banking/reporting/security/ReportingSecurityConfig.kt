...
class ReportingSecurityConfig(
    @param:Value("\${banking-lab.security.enabled:true}")
    private val enabled: Boolean
) {
    @Bean
    fun reportingSecurityFilterChain(
        http: HttpSecurity,
        entryPoint: ReportingAuthenticationEntryPoint,
        jwtAuthenticationConverter: ReportingJwtAuthenticationConverter
    ): SecurityFilterChain {
    ...
    fun reportingJwtDecoder(
        ...
        ReportingResourceServerJwtDecoder(
            objectMapper,
            jwksUri,
            expectedIssuer,
            expectedAudience,
