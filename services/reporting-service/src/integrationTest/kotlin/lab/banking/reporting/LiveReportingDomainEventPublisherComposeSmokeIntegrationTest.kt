...
class LiveReportingDomainEventPublisherComposeSmokeIntegrationTest {
    private val objectMapper = jacksonObjectMapper()

    @Test
    fun `live reporting domain event publisher emits pending report event from Compose worker`() {
        val composeProject = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_COMPOSE_PROJECT").orEmpty()
        assumeTrue(
            composeProject.isNotBlank(),
            "set BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_COMPOSE_PROJECT to run live reporting publisher drill"
        )

        val topic = System.getenv("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_TOPIC")
            ?: "banking.lab.reporting-domain-publisher-smoke"
        val databaseUrl = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_DATABASE_URL")
            ?: "jdbc:postgresql://127.0.0.1:${System.getenv("BANKING_LAB_POSTGRES_PORT") ?: "5432"}/banking_lab"
        val databaseUser = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_DATABASE_USER") ?: "banking_lab"
        val databasePassword = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_DATABASE_PASSWORD") ?: "banking_lab"
        val bootstrapServers = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_BOOTSTRAP_SERVERS")
            ?: "127.0.0.1:${System.getenv("BANKING_LAB_REDPANDA_PORT") ?: "9092"}"
        val publisherService = System.getenv("BANKING_LAB_LIVE_REPORTING_DOMAIN_PUBLISHER_SERVICE")
            ?: "reporting-domain-event-publisher"
        val composeEnv = composeEnvironment(topic)
        val artifactId = "RPT-LIVE-${UUID.randomUUID().toString().uppercase()}"
        val outboxEventId = "RPO-LIVE-${UUID.randomUUID().toString().uppercase()}"
        val idempotencyKey = "IDEMP-LIVE-REPORTING-${UUID.randomUUID().toString().uppercase()}"

        waitForReportingOutboxSchema(databaseUrl, databaseUser, databasePassword, Duration.ofSeconds(90))
        ...
        try {
            ...
            assertEquals("PENDING", outboxStatus(databaseUrl, databaseUser, databasePassword, outboxEventId))
            ...
            assertEquals(outboxEventId, envelope.outboxEventId)
            assertEquals("REPORT_ARTIFACT", envelope.aggregateType)
            assertEquals(artifactId, envelope.aggregateId)
            assertEquals("ReportArtifactGenerated", envelope.eventType)
            assertEquals(idempotencyKey, envelope.idempotencyKey)
            assertEquals(true, envelope.headers["syntheticOnly"])
            assertEquals("reporting-service", envelope.headers["sourceService"])
            assertEquals(artifactId, envelope.payload["artifactId"])
            ...
            assertTrue(logLine.contains("observability.reporting.publisher"), logLine)
    ...
    private fun waitForReportingOutboxSchema(
        databaseUrl: String,
        databaseUser: String,
            ...
            try {
                connection(databaseUrl, databaseUser, databasePassword).use { conn ->
                    conn.prepareStatement(
                        "SELECT to_regclass('public.reporting_outbox_events'), to_regclass('public.idx_reporting_outbox_status_retry')"
                    ).use { stmt ->
                        stmt.executeQuery().use { rs ->
                            if (rs.next() && rs.getString(1) == "reporting_outbox_events" && rs.getString(2) != null) {
                                return
                            }
                        }
                    }
        ...
        fail<Unit>("reporting_outbox_events schema was not ready: ${lastError?.message}")
    }

    private fun insertReportingOutboxEvent(
        databaseUrl: String,
        databaseUser: String,
        databasePassword: String,
        ...
        connection(databaseUrl, databaseUser, databasePassword).use { conn ->
            conn.prepareStatement(
                ...
                VALUES (?, 'ReportArtifactGenerated', 'REPORT_ARTIFACT', ?, ?, CAST(? AS jsonb), 'PENDING', true)
                """.trimIndent()
            ).use { stmt ->
                ...
                stmt.setString(
                    ...
                    """
                    {
                      ...
                      "drill": "compose-reporting-domain-publisher"
                    }
                    """.trimIndent()
                )
                stmt.executeUpdate()
    ...
    private fun queryOutboxValue(
        ...
        connection(databaseUrl, databaseUser, databasePassword).use { conn ->
            conn.prepareStatement("SELECT $columnExpression FROM reporting_outbox_events WHERE outbox_event_id = ?").use { stmt ->
                stmt.setString(1, outboxEventId)
                stmt.executeQuery().use { rs ->
                    if (rs.next()) {
                        rs.getString(1)
    ...
    private fun waitForKafkaEnvelope(
        ...
        val props = Properties().apply {
            ...
            put(ConsumerConfig.GROUP_ID_CONFIG, "live-reporting-domain-publisher-${UUID.randomUUID()}")
            put(ConsumerConfig.CLIENT_ID_CONFIG, "live-reporting-domain-publisher-consumer-${UUID.randomUUID()}")
        ...
        return fail("reporting domain event publisher did not produce a record for $outboxEventId on $topic")
    }
    ...
    private fun waitForPublisherBatchLog(
        ...
        while (System.nanoTime() < deadline) {
            ...
            val matchingLine = lastOutput
                .lineSequence()
                .lastOrNull {
                    it.contains("observability.reporting.publisher") &&
        ...
        return fail("reporting publisher batch log for topic $topic was not found:\n$lastOutput")
    }
    ...
    private fun composeEnvironment(topic: String): Map<String, String> =
        buildMap {
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_TOPIC", topic)
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_CLIENT_ID", "reporting-compose-domain-event-publisher-smoke")
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_POLL_INTERVAL_MILLIS", "250")
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_TIMEOUT_MILLIS", "5000")
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_DEAD_LETTER_THRESHOLD", "3")
            put("BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_RETRY_DELAY_SECONDS", "60")
            put(
                "BANKING_LAB_REPORTING_DOMAIN_EVENT_PUBLISHER_EVENT_TYPES",
                "ReportArtifactGenerated,ReportArtifactExported,ReportRetentionSweepCompleted"
            )
