...
object NotificationJwtClaims {
    fun toPrincipal(claims: Map<String, Any?>, fallbackRoles: Set<String> = emptySet()): NotificationPrincipal? {
...
@Component
class NotificationJwtAuthenticationConverter : Converter<Jwt, AbstractAuthenticationToken> {
    override fun convert(jwt: Jwt): AbstractAuthenticationToken {
        val authorities = NotificationJwtClaims.rolesFromClaims(jwt.claims)
            .flatMap { listOf(SimpleGrantedAuthority("ROLE_$it"), SimpleGrantedAuthority("SCOPE_$it")) }
            .toSet()
        return JwtAuthenticationToken(jwt, authorities, NotificationJwtClaims.subjectFromClaims(jwt.claims) ?: jwt.subject)
    }
}

class NotificationResourceServerJwtDecoder(
    ...
    private fun decodeSimulatorToken(token: String): Jwt {
        ...
        return Jwt.withTokenValue(token)
            ...
            .issuer(claims["iss"]?.toString())
            .subject(NotificationJwtClaims.subjectFromClaims(claims))
            .issuedAt(NotificationJwtClaims.epochInstant(claims["iat"]) ?: now)
            .expiresAt(NotificationJwtClaims.epochInstant(claims["exp"]) ?: now.plusSeconds(300))
    ...
    private fun validateClaims(claims: Map<String, Any?>, requireExpiry: Boolean = true) {
        ...
        if (expectedAudience.isNotBlank() && !NotificationJwtClaims.stringSet(claims["aud"]).contains(expectedAudience)) {
            throw BadJwtException("token audience is not allowed")
        }
        if (NotificationJwtClaims.subjectFromClaims(claims) == null) {
            throw BadJwtException("token subject is required")
        }
        if (NotificationJwtClaims.rolesFromClaims(claims).isEmpty()) {
            throw BadJwtException("token role claim is required")
        }
        if (requireExpiry && NotificationJwtClaims.epochInstant(claims["exp"]) == null) {
    ...
    private fun normalizedRegisteredClaims(claims: Map<String, Any?>): Map<String, Any?> =
        claims.mapValues { (key, value) ->
            when (key) {
                "iat", "exp", "nbf" -> NotificationJwtClaims.epochInstant(value) ?: value
                else -> value
            }
        }
}
