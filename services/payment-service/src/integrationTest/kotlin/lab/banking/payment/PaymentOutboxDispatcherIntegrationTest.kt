...
class PaymentOutboxDispatcherIntegrationTest {
    ...
    fun `dispatcher posts pending ledger outbox through core port and records settlement`() {
        ...
        assertEquals(PaymentInstructionStatus.LEDGER_POSTED, settled.status)
        ...
        assertEquals(1, countRows("payment_outbox_events WHERE event_type = 'PaymentInstructionLedgerPosted' AND status = 'PENDING'"))
        assertEquals(1, countRows("payment_attempts WHERE payment_instruction_id = '${created.item.paymentInstructionId}' AND status = 'LEDGER_POSTED'"))
    }
    ...
    fun `dispatcher dead letters ledger outbox after threshold without settling instruction`() {
        ...
        assertEquals(0, countRows("payment_outbox_events WHERE event_type = 'PaymentInstructionLedgerPosted'"))
