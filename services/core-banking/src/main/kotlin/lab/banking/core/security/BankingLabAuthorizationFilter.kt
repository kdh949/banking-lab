...
class BankingLabAuthorizationFilter(
    ...
    private fun screenId(path: String): String =
        when {
            ...
            path == "/api/customer/me" -> "CWB-003"
            path.startsWith("/api/customer/account-opening-requests") -> "CWB-004"
            path == "/api/customer/360" -> "CWB-104"
            path.startsWith("/api/customer/accounts/") && path.contains("/statement") -> "CWB-105"
            path.startsWith("/api/customer/statements/consolidated") -> "CWB-106"
            path.startsWith("/api/customer/statements/artifacts") -> "CWB-107"
